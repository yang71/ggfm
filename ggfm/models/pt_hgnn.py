...
class PT_HGNN(nn.Module):
    ...
    def link_loss(self, node_emb, rem_edge_list, ori_edge_list, node_dict, target_type, use_queue=True,
                  update_queue=False, relation_level=False):
        losses = 0
        ress = []
        ...
        for source_type in rem_edge_list:
            ...
            for relation_type in rem_edge_list[source_type]:
                ...
                if relation_level:
                    ...
                    negative_source_ids = to2Darr(
                        [idxs[m][:sn] for idxs, m in zip(negative_source_ids, masks.cpu().numpy())])
    ...
    def structure_loss(self, node_emb, rem_edge_list, ori_edge_list, node_dict, target_type, use_queue=True,
                       update_queue=False, relation_level=False):
        losses = 0
        # ress = []
        ...
        for source_type in rem_edge_list:
            ...
            for relation_type in rem_edge_list[source_type]:
                ...
                for edge in rem_edges:
                    node_nbr_dict[edge[0] + node_dict[target_type][0]][source_type][relation_type].append(
                        edge[1] + node_dict[source_type][0])
        ...
        for idx, target_id in enumerate(target_node_ids):
            ...
            for source_type in node_nbr_dict[target_id]:
                ...
                for relation_type in node_nbr_dict[target_id][source_type]:
                    ...
                    tar_schema_emb = torch.cat([tar_schema_emb, structure_map(node_emb[torch.LongTensor(tmp_ids)])],
                                               dim=0)
        ...
        self.neg_structure_queue = torch.cat([schema_emb.detach(), self.neg_structure_queue], dim=0)[
                                   :self.neg_structure_queue_size]
