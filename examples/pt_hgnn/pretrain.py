...
from ggfm.models.pt_hgnn import GNN, PT_HGNN
...
'''
    ...
    Dataset arguments
'''
    parser.add_argument('--data_dir', type=str, default='dataset/graph_CS.pk',
                        help='The address of preprocessed graph.')
...
'''
    parser.add_argument('--use_relation', help='Whether to use relation level task', action='store_true')
...
def train():
    global data, node_feature, edge_index, gnn, pthgnn
    ...
    pthgnn = PT_HGNN(gnn=gnn, rem_edge_list=rem_edge_list, attr_decoder=attr_decoder, \
                     neg_queue_size=0, types=types, neg_samp_num=args.neg_samp_num, device=device)
    pthgnn.init_emb.data = node_feature[node_type == node_dict[target_type][1]].mean(dim=0).detach()
    pthgnn = pthgnn.to(device)
    best_val = 100000
    train_step = 0
    stats = []
    optimizer = torch.optim.AdamW(pthgnn.parameters(), weight_decay=1e-2, eps=1e-06, lr=args.max_lr)
    ...
    for epoch in np.arange(args.n_epoch) + 1:
        pthgnn.neg_queue_size = args.queue_size * epoch // args.n_epoch
        for batch in np.arange(repeat_num) + 1:
            ...
            for data, rem_edge_list, ori_edge_list, attr, (start_idx, end_idx) in train_data:
                ...
                node_feature[start_idx: end_idx] = pthgnn.init_emb
                node_emb = pthgnn.gnn(node_feature.to(device), node_type.to(device), edge_time.to(device), \
                                      edge_index.to(device), edge_type.to(device))

                loss_link, _ = pthgnn.link_loss(node_emb, rem_edge_list, ori_edge_list, node_dict, target_type,
                                                use_queue=True, update_queue=True, relation_level=args.use_relation)

                if args.use_text:
                    if args.attr_type == 'text':
                        loss_attr = pthgnn.text_loss(node_emb[start_idx: end_idx], attr, w2v_model, device)
                    else:
                        loss_attr = pthgnn.feat_loss(node_emb[start_idx: end_idx], torch.FloatTensor(attr).to(device))
                ...
                optimizer.zero_grad()
                loss.backward()
                torch.nn.utils.clip_grad_norm_(pthgnn.parameters(), args.clip)
                optimizer.step()
            ...
            with torch.no_grad():
                ...
                node_emb = pthgnn.gnn(node_feature.to(device), node_type.to(device), edge_time.to(device), \
                                      edge_index.to(device), edge_type.to(device))
                loss_link, ress = pthgnn.link_loss(node_emb, rem_edge_list, ori_edge_list, node_dict, target_type,
                                                   use_queue=False, update_queue=True, relation_level=args.use_relation)
                ...
                if args.use_text:
                    print((
                          ...
                          (epoch, batch, repeat_num, (st - et), optimizer.param_groups[0]['lr'],
                           np.average(train_link_losses), np.average(train_attr_losses), \
                           loss_link, loss_attr, np.average(ndcgs), node_emb.norm(dim=1).mean(), pthgnn.neg_queue_size))
                else:
                    print((
                              "Epoch: %d, (%d / %d) %.1fs  LR: %.5f Train Loss: (%.3f)  Valid Loss: (%.3f)  NDCG: %.3f  Norm: %.3f  queue: %d") % \
                          (epoch, batch, repeat_num, (st - et), optimizer.param_groups[0]['lr'],
                           np.average(train_link_losses), \
                           loss_link, np.average(ndcgs), node_emb.norm(dim=1).mean(), pthgnn.neg_queue_size))

            if valid_loss < best_val:
                best_val = valid_loss
                print('UPDATE!!!')
                torch.save(pthgnn.state_dict(), args.pretrain_model_dir)
