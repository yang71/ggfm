    ...
    def __init__(self,
        ...
        if self.task == "nc":
            for i, id in enumerate(self.idx):
                ...
                l["question"] = f"Given a node-centered graph: <qraph>, each node represents a paper, we need to classify the center node into {classes} classes. please tell me which class the center node belongs to?"
                l["label"] = f"{label}"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=str, default='/home/yjy/heteroPrompt/system/ggfm/fine_tuned_model',
                        help='The address for storing the models and optimization results.')
    parser.add_argument('--task_name', type=str, default='gptgnn_nc',
    ...
    parser.add_argument('--task', type=str, default='lp', help='task')
    ...
    parser.add_argument('--num_epochs', type=int, default=1, help='Number of epoch to run')
    parser.add_argument('--batch_size', type=int, default=16, help='Number of output nodes for training')
    parser.add_argument('--output_dir', type=str, default='/home/cwj/tmp_lp',help='saving path')
    parser.add_argument('--result_save_dir', type=str, default='/home/cwj/tmp_lp/',help='saving path' )
    ...
    ylabel_train = generate_lp_labels(graph, train_ids, train_pairs, output_num)
    ylabel_valid = generate_lp_labels(graph, valid_ids, valid_pairs, output_num)
    ylabel_test = generate_lp_labels(graph, test_ids, test_pairs, output_num)
    ...
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, drop_last=False, pin_memory=True,
                              collate_fn=collate_fn)
    ...
    res = np.zeros_like(ylabel_test)
    for i in range(len(predictions)):
        for j in range(len(predictions[i])):
            res[i][predictions[i][j]] = 1

    test_res = []
    for ai, bi in zip(ylabel_test, np.argsort(-res)):
        test_res += [ai[bi]]
